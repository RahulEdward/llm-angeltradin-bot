"""
SmartWebSocketV2 - Angel One WebSocket for Real-time Market Data

This provides real-time streaming data for:
- LTP (Last Traded Price)
- Quote (with OHLC, Volume)
- Snap Quote (detailed with depth)
- Depth (Order book - 20 levels)

Usage:
    ws = SmartWebSocketV2(auth_token, api_key, client_code, feed_token)
    ws.on_data = lambda wsapp, data: print(data)
    ws.on_open = lambda wsapp: ws.subscribe("abc123", 1, [{"exchangeType": 1, "tokens": ["10626"]}])
    ws.connect()
"""
...
class SmartWebSocketV2:
    ...
    def _on_pong(self, wsapp, data):
        if data == self.HEART_BEAT_MESSAGE:
            ...
            logger.debug(f"In on pong function ==> {data}, Timestamp: {formatted_timestamp}")
            self.last_pong_timestamp = timestamp

    def _on_ping(self, wsapp, data):
        timestamp = time.time()
        formatted_timestamp = time.strftime("%d-%m-%y %H:%M:%S", time.localtime(timestamp))
        logger.debug(f"In on ping function ==> {data}, Timestamp: {formatted_timestamp}")
    ...
    def on_message(self, wsapp, message):
        """Called when a non-binary message is received (like pong)"""
        pass
    ...
    def on_open(self, wsapp):
        """Called when connection is opened - subscribe to tokens here"""
        pass
