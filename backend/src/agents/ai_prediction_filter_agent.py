...
class AIPredictionFilter:
    ...
    def check_divergence(self, 
        ...
        Returns:
            Dict with allow_trade, reason, confidence_boost, ai_veto,
        ...
        logger.info(f"🤖 AI Filter Check: Trend={trend_1h}, AI={ai_signal} ({prob_up:.1%}), Conf={ai_conf:.1%}")
        ...
        if (trend_1h == 'long' and ai_signal == 'bullish') or \
           (trend_1h == 'short' and ai_signal == 'bearish'):
            logger.info(f"✅ Perfect Trend-AI Resonance: {trend_1h.upper()} + AI {ai_signal.upper()}")
            return {
                'allow_trade': True,
        ...
        if ai_conf > self.veto_threshold:
            logger.warning(f"🚫 AI VETO: Trend={trend_1h} but AI predicts {ai_signal} with {ai_conf:.0%} confidence")
            return {
                'allow_trade': False,
        ...
        logger.warning(f"⚠️ AI-Trend Divergence: {trend_1h} vs {ai_signal}, but confidence only {ai_conf:.0%}")
        return {
            'allow_trade': True,
    ...
    def get_resonance_quality(self, 
        ...
        Returns: 'perfect' | 'divergence' | 'neutral' | 'uncertain'
        """
